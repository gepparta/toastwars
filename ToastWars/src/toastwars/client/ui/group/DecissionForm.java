...
public class DecissionForm extends Panel
{
	...
	public DecissionForm(ArrayList<Button> buttons, NumberField capital, Object o, ArrayList<Toaster> newToasterList)
	{
		setBorder(false);
	...
	private void createEmptyContent()
	{
		...
		if (type == Type.TYPE2)
			...
			costs = 25000;
		HTMLPanel label = new HTMLPanel("<p>" + "Die Entwicklung dieses Typs kostet " + NumberUtil.formatToEuro(costs) + ".</p>");
		label.setStyle("background: black;");
		label.setWidth(250);

		newPanel.add(label);

		newButton = new Button(type.getDescription() + " entwickeln", new ButtonListenerAdapter()
		{
			public void onClick(Button button, EventObject e)
			{
				...
				try
				{
					toaster.setPrice((type.getMinPrice() + type.getMaxPrice()) / 2);
				} catch (Exception e1)
				{
					e1.printStackTrace();
				}
				...
				capital.setValue(capital.getValue().doubleValue() - costs);
	...
	private Panel createTopPanel()
	{
		...
		// add price field
		SliderBar priceSlider = new SliderBar(type.getMinPrice(), type.getMaxPrice());
		int ticks = 15;
		if (type == Type.TYPE3)
			ticks = 7;
		int stepSize = (type.getMaxPrice() - type.getMinPrice()) / ticks;
		configureSlider(priceSlider, stepSize, ticks, 4, " &euro;", fields.get(0), toaster.getPrice());
		...
		configureSlider(amountSlider, type.getMarketVolume() / 20, 20, 2, " ME", fields.get(1), toaster.getProduction());
	...
	private void createFields()
	{
		...
		fields.add(createNumberField("Preis", "price", toaster.getPrice(), toaster.getType().getMinPrice(), toaster.getType().getMaxPrice(), true));
		fields.add(createNumberField("Menge", "amount", toaster.getProduction(), 0, toaster.getType().getMarketVolume(), false));
		fields.add(createNumberField("Zeitung", "mag", toaster.getNewspaperInvestment(), 0, 0, false));
		fields.add(createNumberField("Radio", "radio", toaster.getRadioInvestment(), 0, 0, false));
		fields.add(createNumberField("TV", "tv", toaster.getTvInvestment(), 0, 0, false));
		fields.add(createNumberField("Qualit&auml;t", "quality", toaster.getQualityInvestment(), 0, 0, false));
		fields.add(createNumberField("Design", "design", toaster.getDesignInvestment(), 0, 0, false));
		fields.add(createNumberField("&Ouml;kologie", "ecology", toaster.getEcologyInvestment(), 0, 0, false));
	}
	...
	private FieldSet createMarketingFieldSet()
	{
		...
		configureSlider(magSlider, value, 20, 2, " &euro;", fields.get(2), toaster.getNewspaperInvestment());
		...
		configureSlider(radioSlider, value, 20, 2, " &euro;", fields.get(3), toaster.getRadioInvestment());
		...
		configureSlider(tvSlider, value, 20, 2, " &euro;", fields.get(4), toaster.getTvInvestment());
	...
	private FieldSet createResearchFieldSet()
	{
		...
		configureSlider(qSlider, value, 20, 2, " &euro;", fields.get(5), toaster.getQualityInvestment());
		...
		configureSlider(designSlider, value, 20, 2, " &euro;", fields.get(6), toaster.getDesignInvestment());
		...
		configureSlider(ecoSlider, value, 20, 2, " &euro;", fields.get(7), toaster.getEcologyInvestment());
	...
	private void configureSlider(SliderBar slider, int stepSize, int ticks, int labels, String postfix, NumberField field, Number value)
	{
		...
		slider.addChangeListener(new SliderChangeListener(field, slider, capital));
		sliders.add(slider);
	}

	private NumberField createNumberField(String text, String name, Number value, int minvalue, int maxvalue, boolean decimal)
	{
		NumberField numField = new NumberField(text, name, 55, value.floatValue());
		...
		numField.setMinText("Der minimale Wert f&uuml;r dieses Feld ist: " + minvalue);
		numField.setMaxText("Der maximale Wert f&uuml;r dieses Feld ist: " + maxvalue);
		...
		numField.addListener(new FieldListenerAdapter()
		{
			...
			public void onValid(Field field)
			{
				...
				Status status = ((Group) Controller.getInstance().getUser()).getStatus();
	...
	private Panel createStockField()
	{
		...
		NumberField stockField = new NumberField("Lagerbestand", "stock", 55, company.getStock().getStock(type));
	...
	public void updateToasterData()
	{
		if (toaster != null && fields.get(0).getValue() != null)
		{
			try
			{
				...
				toaster.setNewspaperInvestment(fields.get(2).getValue().doubleValue());
				toaster.setRadioInvestment(fields.get(3).getValue().doubleValue());
				toaster.setTvInvestment(fields.get(4).getValue().doubleValue());
				toaster.setQualityInvestment(fields.get(5).getValue().doubleValue());
				toaster.setDesignInvestment(fields.get(6).getValue().doubleValue());
				toaster.setEcologyInvestment(fields.get(7).getValue().doubleValue());
				// by Alex
				// an dieser Stelle wird wie in dem Szenario beschrieben
				// eine Invetsition in Ökologie in Runde 2 mit 20% bezuschusst
				if (Controller.getInstance().getGame().getCurrentRound() == 2 && ((Group) Controller.getInstance().getUser()).getStatus() == Status.COMPLETED)
				{
