...
public class Game {
	...
	public Game (int userAmount){
		this.userAmount = userAmount;
		this.currentRound = 1;
		this.companyList = createInitialData(userAmount,1.00,1,1,1.00,1,1,1.00,1,1,"Comp",1.00,1.00,1.00);
				}
	...
	private ArrayList<Company> createInitialData (int userAmount, 
			...
			String initialDescription, double initialTurnover, double initialMarketShare, double initialAsset)
			{
			Type Typ1 = Type.type1;
			Type Typ2 = Type.type2;
			Type Typ3 = Type.type3;
			
			Toaster Toaster1 = new Toaster (initialPrice1,initialMarketing1,initialResearch1,Typ1);
			Toaster Toaster2 = new Toaster (initialPrice2,initialMarketing2,initialResearch2,Typ2);
			Toaster Toaster3 = new Toaster (initialPrice3,initialMarketing3,initialResearch3,Typ3);
			...
			companyList.add(new Company(initialDescription, initialTurnover,initialMarketShare,initialAsset,toasterList));
	...
	public void simulate (int marketVolume, int fixedCost, int variableCost){
		...
		for (int i=0; i==companySize;i++){
			index[i] = companyList.get(i).getToasterList().get(1).calculateIndex(random);
			IndexSum=IndexSum + index[i];
			}//for
		
		//calculate MarketShare
		for (int i=0; i==companySize;i++){
			marketShare[i] = (int)Math.round(index[i]/IndexSum * marketVolume);
		...
		for (int i=0; i==companySize;i++){
			turnover[i] = (int) Math.round(marketShare[i] * companyList.get(i).getToasterList().get(1).getPrice());
			companyList.get(i).setTurnover(turnover[i]);
		}
		//calculate cost
		
		for (int i=0; i==companySize;i++){
			cost[i] = (int) Math.round(marketShare[i] * variableCost) + fixedCost;
		...
		for (int i=0; i==companySize;i++){
			asset[i] = (int) Math.round(companyList.get(i).getAsset()+companyList.get(i).getTurnover()-cost[i]);
	...
	public void changePrice(Company company, Toaster toaster, double price){
		...
		public static void main (String[] args){
			...
			G1.changePrice(G1.getCompanyList().get(0), G1.getCompanyList().get(0).getToasterList().get(0), 100);
